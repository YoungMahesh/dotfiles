...
CHROMIUM_COUNT=$(i3-msg -t get_tree | jq '.nodes[] | .. | select(.type? == "workspace" and .name? == "2") | .. | select(.window_properties?.class? == "Google-chrome") | .window_properties.class' | wc -l)
...
if [ "$CHROMIUM_COUNT" -eq 0 ]; then
    ...
    # get profile-path by visiting "chrome://version/" inside chromium
